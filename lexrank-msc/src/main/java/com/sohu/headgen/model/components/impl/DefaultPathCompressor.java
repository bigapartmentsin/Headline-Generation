package com.sohu.headgen.model.components.impl;
...
import com.sohu.headgen.model.components.GraphModel;
...
public final class DefaultPathCompressor implements PathCompressor {
    ...
    public Optional<String> compress(GraphDatabaseService graph, int maxDepth) {
        try (Transaction tx = graph.beginTx()) {
            ...
            Iterable<Path> allPaths = finder.findAllPaths(GraphModel.start(graph), GraphModel.end(graph));
            for (Path path : allPaths) {
                boolean hasVerb = PathCompressor.hasVerb(path);
                if (path.length() >= PathCompressor.MIN_DEPTH && hasVerb) {
                    double cost = 0.0;
                ...
                total += 1;
                if(total >= PathCompressor.MAX_PATH_NUM) {
                    break;
                }
            }
